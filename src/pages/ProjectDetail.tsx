...
export default function ProjectDetail() {
  ...
  useEffect(() => {
    ...
    const foundProject = typedData.projects.find((p) => p.slug === slug);
    if (foundProject) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const descriptionTop = descriptionRef.current?.offsetTop ?? 0;
      const descriptionHeight = descriptionRef.current?.offsetHeight ?? 1;
      ...
      if (scrollY >= descriptionTop - windowHeight / 2) {
        ...
        const images =
          galleryRef.current?.querySelectorAll<HTMLImageElement>("img");
        images?.forEach((img, index) => {
          const offset = (index + 1) * 80;
          const translateY = progress * offset;
          img.style.transform = `translateY(${translateY}px)`;
          img.style.opacity = String(1 - progress * 0.2);
        });
      }
    };
          ...
          <div className="main-image-container">
            <img
              ...
              onError={(e) => {
                ...
                if (parent) {
                  parent.innerHTML =
                    '<div class="image-placeholder">Image principale</div>';
                }
              }}
            />
          </div>
                ...
                <div key={index} className="gallery-item">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      const parent = img.parentElement;
                      if (parent) {
                        parent.innerHTML = `<div class=\"image-placeholder\">Image ${index + 1}</div>`;
                      }
                    }}
                  />
                </div>
